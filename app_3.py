...
"Shayan Ahmed Viringipuram",
"Shifhana Banu Usain",
"Shyam Sundar",
"Sivasankari Arumugam",
"Sneha Hari Doss",
"Somalakshmi Dhanachezhiyan",
"Sonia Selva Kumar",
"Sridevi Rangarajan",
"Sruthi Mathivanan",
"Steffin T M",
"Sudha Birendarkumar",
"Sudharshan Kumaresan",
"Swetha Mani",
...
"Vinod Ram",
"Vishwa Alagiri",
"Vyshnavi Sunami Koniki",
"Yashvanth M S",
"Yathin T",
"Yazhini Krishnamoorthy",
"Yugeshwaran Aroumougam",
"Yuvaraj Selvam",
...
def clean_tracker(raw_df: pd.DataFrame,
    ...
    #    Non-numeric text (e.g. "shantha") → keep value but flag red.
    #    Empty → "-"
    def clean_tim(val):
        if is_empty(val):
            return "-", False
        s = str(val).strip()
        # Float like 4494128.0 → integer string
        try:
            f = float(s.replace(" ", ""))
            if f == int(f):
                return str(int(f)), False   # valid numeric TIM
        except ValueError:
            pass
        cleaned = s.replace(" ", "")
        # Pure digits after space removal = valid
        if cleaned.isdigit():
            return cleaned, False
        # Anything else = unwanted text → red
        return cleaned, True

    tim_results = df["TIM#"].apply(clean_tim)
    df["TIM#"]  = tim_results.apply(lambda x: x[0])
    for i, (_, red) in enumerate(tim_results):
        if red:
            red_cells.add((i, "TIM#"))
    ...
    # ── 8b. SET lINE ─────────────────────────────────────────────────────────
    #    Blank → 0. Alphabets/words → 0. Numbers → kept as-is.
    def clean_set_line(val):
        if is_empty(val):
            return 0
        try:
            return int(float(str(val).replace(",", "").strip()))
        except ValueError:
            return 0   # alphabets / words → 0

    df["Set line"] = df["Set line"].apply(clean_set_line)
    ...
    def should_drop(i, row):
        for col in DROP_COLS:
            if str(row[col]).strip() == "-":
                return True
        if i in red_row_indices:
            return True
        return False
...
def build_pivot(df: pd.DataFrame) -> pd.DataFrame:
    """Name | Count of TIM# | Sum of MG Lines | Sum of Oversold Lines | Sum of Set line | Total Lines"""
    pivot = (
    ...
    pivot["Total Lines"] = (
        pivot["Sum of MG Lines"] + pivot["Sum of Oversold Lines"] + pivot["Sum of Set line"]
    )
    ...
    grand = pd.DataFrame([{
        ...
        "Sum of Set line":      pivot["Sum of Set line"].sum(),
        "Total Lines":           pivot["Total Lines"].sum(),
    }])
...
with st.expander("📋 Cleaning rules reference"):
    ...
    <div class="rules-wrap">
      ...
      <div class="rule-pill"><b>Set line</b>Blank or words → 0 · numbers kept as-is</div>
